...
public class diff_match_patch {
        ...
        while (thisDiff != null) {
            if (thisDiff.operation == Operation.EQUAL) {
                ...
                if (lastequality != null &&
                        ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.length() < Diff_EditCost / 2) && ((pre_ins ? 1 : 0) +
    ...
    protected void patch_addContext(final Patch patch, final String text) {
        ...
        // Add the prefix.
        final String prefix = text.substring(Math.max(0, patch.start2 - padding), patch.start2);
        if (prefix.length() != 0) {
            patch.diffs.addFirst(new Diff(Operation.EQUAL, prefix));
        }
        // Add the suffix.
        final String suffix = text.substring(patch.start2 + patch.length1, Math.min(text.length(), patch.start2 + patch.length1 + padding));
        if (suffix.length() != 0) {
     ...
     */
    @Deprecated
    public LinkedList<Patch> patch_make(final String text1, final String text2, final LinkedList<Diff> diffs) {
        ...
        for (final Patch aPatch : patches) {
            ...
            if (start_loc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    if (text1.length() > Match_MaxBits && diff_levenshtein(diffs) / (float) text1.length() > Patch_DeleteThreshold) {
    ...
    public String patch_addPadding(final LinkedList<Patch> patches) {
        ...
        final short paddingLength = Patch_Margin;
        String nullPadding = "";
     ...
     *             If invalid input.
     */
    public List<Patch> patch_fromText(final String textline) throws IllegalArgumentException {
    ...
    public static class Diff {
        ...
        public String toString() {
            ...
            final String prettyText = text.replace('\n', '\u00b6');
